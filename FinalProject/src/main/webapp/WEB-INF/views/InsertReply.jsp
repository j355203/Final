...
<head>
	...
	<title>回覆：${AB.topic}</title>


	<script>
		document.addEventListener("DOMContentLoaded", function () {
			//事件繫結
			document.getElementById("replyContent").addEventListener("blur", checkReplyContent);
		});

		function checkReplyContent() {
			let theReplyContent = document.getElementById("replyContent").value;
			let theReplyContentLen = theReplyContent.length;

			if (theReplyContent == "") {
				// alert("你不可以輸入空白");
				document.getElementById("sp2").innerHTML = "<img src='${pageContext.request.contextPath}/Image/Wrong-s.png'> 你不可以輸入空白!";
			}
			else if (theReplyContent > 1000) {
				// alert("你不可以輸入超過1000字");
		...
		<div class="container-fluid mt--8">
			...
			<div class="row">
				<div class="row justify-content-md-center">
					<div class="col-lg-10">
						<form:form method="POST" modelAttribute="InsertReply">
							<fieldset>
								<!-- <legend>回覆討論文章</legend> -->
								<div class="fr1">
									<label for="class" class="lab1">文章標題：</label>
									<input type="hidden" value="${AB.articleId}" name="articleId" path="articleId" />
									<input type="hidden" name="empId" path="empId" value="${RBL.empId}" />
									<input type="hidden" value="${RBL.empName}" name="empName" path="empName" />
									<input class="form-control form-control-alternative" type="text" id="topic"
										name="topic" placeholder="RE:${AB.topic}" value="RE:${AB.topic}" readonly>
								</div>
								<div class="fr1">
									<label for="article" class="lab1">回覆內容：</label>
									<textarea class="form-control form-control-alternative" cols="55" rows="30"
										id="article" name="replyContent" path="replyContent" placeholder="請在此輸入回覆內容"
										maxlength="1000"></textarea>
									<p class="p1">(1.不可空白，2.最多1000字)</p>
									<br>
									<span id="sp2"></span>
								</div>
							</fieldset>
