	...
	<script>
		$(document).ready(function () {
			$("#reportButton").click(function () {
				let reportNum = document.getElementById("reportNum").value;
				if (reportNum == 3) {
					alert("此檢舉已經審核，待刪除!!");
				}
				else {
...
<body>
	...
	<div class="main-content">
		...
		<div class="container-fluid mt--8" ng-app>
			...
			<div class="row">
					...
					<div class="col-lg-12">
						<div align=center class="articleTable">
							<table style="background-color: snow;border-radius:10px;">
								...
								<tbody>
									<tr>
										<td class="articleID">文章編號：${AB.articleId}</td>
							...
							<c:forEach items="${RBL}" var="RBL" varStatus="s">
								<table style="background-color: snow;border-radius:10px;border: 1;">
									<thead>
										<tr ng-show="${sessionScope.emp.empid} == ${RBL.empId}">
											<th class="topic" colspan="2" style="border-right: 1px solid #000080;">
												RE：
												<c:out value="${AB.topic}" />
											</th>
											<th>
												<input style="margin-left: 50px;font-family: Microsoft JhengHei;"
													type="button" class="but1" value="修改回覆"
													onclick="location.href='<spring:url value='/UpdateReply/${RBL.replyId}' />'" />
												<input style="margin-left: 40px;font-family: Microsoft JhengHei;"
				...
				<div class="col-lg-2" style="margin-top: 136px;margin-left: 90px;">
					<input style="margin-bottom: 16px;" class="btn btn-info" type="button" value="回覆文章"
						onclick="location.href='<spring:url value='/InsertReply/${AB.articleId}' />'" />
					<input ng-show="${sessionScope.emp.empid} == ${AB.empId}" style="margin-bottom: 16px;"
						class="btn btn-success" type="button" value="修改文章"
						onclick="location.href='UpdateArticle/${AB.articleId}'" />
					<input ng-show="${sessionScope.emp.empid} == ${AB.empId}" style="margin-bottom: 16px;"
						class="btn btn-warning" type="button" value="刪除文章"
						onclick="location.href='<spring:url value='/DeleteArticle/${AB.articleId}' />'" />
					<input id="reportButton" style="margin-bottom: 16px;" class="btn btn-secondary" type="button"
						value="檢舉文章" />
				</div>
